...
def do_replace(filename, line_num, text):
...
def replace_line(server):
    @server.route('/edit/replace_line', methods=['POST'])
    def _replace_line():
        """
        ---
        post:
            operationId: edit_replace_line
            summary: EASIEST way to edit! Writes new_text to file_name at line_number, replacing the text on that line, moving other lines down. ALWAYS view_file to check line nums 1st! NEVER Use the Shell Tool To Edit!
            requestBody:
                content:
                    application/json:
                        schema:
        ...
        """
        console.verbose("replace_line request")
        ...
        do_replace(request.json['file_name'], request.json['line_number'], request.json['new_text'])
